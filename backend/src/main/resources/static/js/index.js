(function(){
    ...
    function connect(username, token){ //1-1
	...
	connectButton.addEventListener('click', async () => {
	    ...
	    let password = select("#webchat_password").value;

	    if(username == null || username === ''){
		alertMessage('Name cannot be empty!!!', 'bg-danger')
	    } else {
		// First we login (REMEMBER POSTMAN BEFOREHAND)
		...
		console.log(json);
		// Then we connect to STOMP
		connect(username, json.accessToken) //1
